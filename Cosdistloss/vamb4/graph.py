...
def neighbors_dict(
    ...
    # initialize the graph_dict and source, target list by adding self edge
...
def cosine_distance(x1, x2, eps=1e-6):
    ...
    dist = 1-_torch.mm(x1, x2.t()) / (w1 * w2.t()).clamp(min=eps)
    return dist.diag()

def eucli_distance(x1,x2):
    return _torch.cdist(x1,x2).diag()
