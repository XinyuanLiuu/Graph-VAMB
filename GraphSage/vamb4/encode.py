...
def make_dataloader(
    ...
    degree_array.shape = (len(degree_array),1)
    degreestensor = _torch.from_numpy(degree_array).float()
    featuretensor = _torch.cat((depthstensor,tnftensor,weightstensor,degreestensor),dim=1)

    n_workers = 4 if cuda else 1

    data = Data(x=featuretensor,edge_index=edge_index)
    ...
    return dataloader, mask, identifiers
...
class VAE(_nn.Module):
    ...
    def __init__(
        ...
        beta: float = 200.0,
        gamma: float = 1, # control the ratio between (C+S) and D
        ...
        for nin, nout in zip(
            ...
            self.encoderlayers.append(SAGEConv(nin, nout))
            self.encodernorms.append(_nn.BatchNorm1d(nout))

        # Latent layers
        self.mu = SAGEConv(self.nhiddens[-1], self.nlatent)
        self.logsigma = SAGEConv(self.nhiddens[-1], self.nlatent)
    ...
    def _encode(self, tensor: Tensor,edge_index:Tensor) -> tuple[Tensor, Tensor]:
        tensors = list()

        # Hidden layers
        for encoderlayer, encodernorm in zip(self.encoderlayers, self.encodernorms):
            tensor = encodernorm(self.dropoutlayer(self.relu(encoderlayer(tensor,edge_index))))
            tensors.append(tensor)

        # Latent layers
        mu = self.mu(tensor,edge_index)
        ...
        logsigma = self.softplus(self.logsigma(tensor,edge_index))
    ...
    def forward(
        self, depths: Tensor, tnf: Tensor, edge_index: Tensor,batch_size_:int
    ) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        tensor = _torch.cat((depths, tnf), 1)
        mu, logsigma = self._encode(tensor,edge_index)
        latent = self.reparameterize(mu, logsigma)
        latent_ori = latent[:batch_size_] # only take the results of original nodes
        depths_out, tnf_out = self._decode(latent_ori)

        return depths_out, tnf_out, mu, logsigma, latent
    ...
    def calc_loss(
        ...
        nbatch= depths_out.shape[0]
        depths_in = depths_in[:nbatch]
        tnf_in = tnf_in[:nbatch]
        weights = weights[:nbatch].squeeze()
        degrees = degrees[:nbatch]
        ...
        if self.nsamples > 1:
            ...
            ce_weight = (1 - self.alpha) / (_log(self.nsamples) * (1 + self.gamma))
        else:
            ce = (depths_out - depths_in).pow(2).sum(dim=1)
            ce_weight = (1 - self.alpha) / (1 + self.gamma)

        ## 4. GNN-based negative log cosine distance, include positive and negative distance loss
        if use_z:
            lat_encoding = latent
        else:
            lat_encoding = mu
        pos_dist_loss = -_torch.log(graph.cosine_distance(lat_encoding[edge_index[0]],lat_encoding[edge_index[1]]) + 1e-9).sum()
        neg_edge_index = negative_sampling(edge_index,num_neg_samples=nedges * 3,force_undirected=True)
        neg_dist_loss = -_torch.log(1-graph.cosine_distance(lat_encoding[neg_edge_index[0]],lat_encoding[neg_edge_index[1]]) + 1e-9).sum()
        
        degree_weight = (degrees/1).clip(max=1.)
        dist = (pos_dist_loss + neg_dist_loss) / nedges * degree_weight
        dist_loss_weight = self.gamma / (1 + self.gamma)
        dist_loss = dist * dist_loss_weight 

        mu = mu[:nbatch]
        logsigma = logsigma[:nbatch]

        sse = (tnf_out - tnf_in).pow(2).sum(dim=1)
        kld = -0.5 * (1 + logsigma - mu.pow(2) - logsigma.exp()).sum(dim=1)

        sse_weight = self.alpha / (self.ntnf * (1 + self.gamma))
        kld_weight = 1 / (self.nlatent * self.beta)

        reconstruction_loss = ce * ce_weight + sse * sse_weight
        kld_loss = kld * kld_weight

        loss = (reconstruction_loss + kld_loss + dist_loss) * weights 

        return loss.mean(), ce.mean(), sse.mean(), kld.mean(), dist.mean()


    def trainepoch(
        self,
        data_loader: _DataLoader,
        embedings_old: Tensor,
        epoch: int,
        optimizer,
        batchsteps: list[int],
        logfile,
        ...
        embedings_new = _torch.empty_like(embedings_old)
        ...
        for batch_data in data_loader:
            ...
            batch_size_ = batch_data.batch_size
            depths_in = batch_data.x[:,:self.nsamples]
            tnf_in = batch_data.x[:,self.nsamples:self.nsamples+self.ntnf]
            weights = batch_data.x[:,self.nsamples+self.ntnf:-1]
            degrees = batch_data.x[:,-1]
            edge_index = batch_data.edge_index
            indexs = batch_data.input_id
            ...
            if self.usecuda:
                ...
                indexs = indexs.cuda()
                embedings_new = embedings_new.cuda()
                embedings_old = embedings_old.cuda()

            optimizer.zero_grad()

            depths_out, tnf_out, mu, logsigma, latent = self(depths_in, tnf_in,edge_index,batch_size_)


            loss, ce, sse, kld, dist = self.calc_loss(
                depths_in, depths_out, tnf_in, tnf_out, mu, logsigma, weights, degrees,latent,
                edge_index, use_z
            )

            embedings_new[indexs] = mu[:batch_size_].clone().detach()  # add the new mu into new embedings tensor
            ...
            epoch_distloss += dist.data.item()

        if logfile is not None:
            print(
                "\tEpoch: {}\tLoss: {:.6f}\tCE: {:.7f}\tSSE: {:.6f}\tKLD: {:.4f}\tDIST: {:.4f}\tBatchsize: {}".format(
                    epoch + 1,
                    epoch_loss / len(data_loader),
                    epoch_celoss / len(data_loader),
                    epoch_sseloss / len(data_loader),
                    epoch_kldloss / len(data_loader),
                    epoch_distloss / len(data_loader),
        ...
        return data_loader, embedings_new, epoch_loss

    def encode(self, data_loader, num_neighbors_) -> _np.ndarray:
        ...
        new_data_loader = NeighborLoader(
            data=data_loader.data,
            num_neighbors=num_neighbors_,
        ...
        row = 0
        with _torch.no_grad():
            for batch_data in new_data_loader:

                batch_size_ = batch_data.batch_size
                depths = batch_data.x[:,:self.nsamples]
                tnf = batch_data.x[:,self.nsamples:self.nsamples+self.ntnf]
                edge_index = batch_data.edge_index
                ...
                # Evaluate
                _, _, mu, _,_ = self(depths, tnf, edge_index, batch_size_)
                mu = mu[:batch_size_]
                if self.usecuda:
                    mu = mu.cpu()
    ...
    def trainmodel(
        ...
        ncontigs = dataloader.data.x.shape[0]
        nsamples = self.nsamples
        ...
        if logfile is not None:
            ...
            print("\tWhether use z:", use_z, file=logfile)
        ...
        embeddings_all = _torch.empty((nepochs,ncontigs,self.nlatent))
        embedings_old_ = _torch.zeros((ncontigs,self.nlatent)) # initialize embeding list of epoch k-1

        for epoch in range(nepochs):

            dataloader, embedings_new_, epoch_loss_ = self.trainepoch(
                dataloader, embedings_old_, epoch, optimizer, sorted(batchsteps_set), logfile, num_neighbors_, use_z,
            )

            embeddings_all[epoch,:,:] = embedings_new_.detach().clone()

            if isnan(epoch_loss_):
                break

            else:
                embedings_old_ = embedings_new_.detach().clone()
        ...
        epoch_choose = [0,int(nepochs/2),nepochs-1]

        return embeddings_all[epoch_choose]
