...
def make_dataloader(
    ...
    indextensor = _torch.arange(0,len(identifiers),dtype=_torch.long)

    n_workers = 4 if cuda else 1
    dataset = _TensorDataset(depthstensor, tnftensor, weightstensor, indextensor)
...
class VAE(_nn.Module):
    ...
    def calc_loss(
        ...
        ## 1. cosine similarity between contig and the average of its neighbors
        nbatch= mu.shape[0]
        neighbors_embedding, degree_weights = graph.neighbors_embedding(self.graph_dict,nbatch,self.nlatent,embedings_old,indexs)
        if self.usecuda:
            neighbors_embedding = neighbors_embedding.cuda()
            degree_weights = degree_weights.cuda()

        if use_z:
            lat_encoding = latent
        else:
            lat_encoding = mu
        dist = graph.cosine_distance(lat_encoding,neighbors_embedding) * degree_weights
        dist_loss_weight = self.gamma / (1+self.gamma) * dist_loss_weight
        ...
        return loss.mean(), ce.mean(), sse.mean(), kld.mean(), dist.sum()/degree_weights.sum()
    ...
    def trainepoch(
        ...
        for depths_in, tnf_in, weights, indexs in data_loader:
            ...
            depths_out, tnf_out, mu, logsigma, latent = self(depths_in, tnf_in)

            embedings_new[indexs] = mu.clone().detach()  # add the new mu into new embedings tensor

            loss, ce, sse, kld, dist = self.calc_loss(
                depths_in, depths_out, tnf_in, tnf_out, mu, logsigma, weights, latent,
                indexs, embedings_old, dist_loss_weight, use_z,
            )
    ...
    def encode(self, data_loader) -> _np.ndarray:
        ...
        row = 0
        with _torch.no_grad():
            for depths, tnf, weights, indexs in new_data_loader:
    ...
    def trainmodel(
        ...
        if logfile is not None:
            ...
            print("\tDist loss weight:", dist_loss_weight, file=logfile)
            print("\tDIst loss start point:",dist_start)
        ...
        if self.usecuda:
            embeddings_all = embeddings_all.cuda()

        for epoch in range(nepochs):

            # if epoch in distweightsteps:
            #     dist_loss_weight = dist_loss_weight * 10
            if epoch < dist_start:
                dist_weight = 0
            else:
                dist_weight = dist_loss_weight

            dataloader, embedings_new_, epoch_loss_ = self.trainepoch(
                dataloader, embedings_old_, epoch, optimizer, sorted(batchsteps_set), logfile, dist_weight, use_z,
            )
